...
extern const char *parseStringWriteToFile(const char *str,
...
napi_value ZigparseStringFromNode(napi_env env, napi_callback_info args) {
  ...
  const char *parsed_str =
      parseSnippetFromString(input_str); // Get string from Zig
...
napi_value ZigParseStringWriteToFile(napi_env env, napi_callback_info info) {
  ...
  const char *result_from_zig = parseStringWriteToFile(
